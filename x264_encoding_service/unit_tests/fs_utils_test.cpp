...
{
  std::string dir = xes::current_directory();
#if PRINT
  std::cout << "current directory: " << dir << std::endl;
#endif

  std::string abs = xes::absolute_path(dir.c_str());
  assert(dir == abs);
}  
  
void test_absolute_path(char const* path)
{
  std::string abs1 = xes::absolute_path(path);
#if PRINT
  std::cout << path << " -> " << abs1 << std::endl;
#endif

  std::string abs2 = xes::absolute_path(abs1.c_str());
  assert(abs1 == abs2);
}
