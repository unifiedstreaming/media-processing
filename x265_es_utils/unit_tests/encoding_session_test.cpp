...
#include <x265_es_utils/encoding_session.hpp>
...
{
  x265_es_utils::encoder_settings_t encoder_settings;
  ...
  x265_es_utils::encoding_session_t session(
  ...
  {
    auto hash = common::fnv1a32::hash(sample_headers.vps_.data(),
      sample_headers.vps_.size());
    *msg << __func__ << ": vps size=" << sample_headers.vps_.size() <<
  ...
  std::ofstream ofs("encoding_session_test.265", std::ios::binary);
  ofs << sample_headers.vps_;
