...
#include <x265_proto/default_endpoints.hpp>
...
{
  return
R"(Copyright (C) 2026 CodeShop B.V.
...
config_t::create_service(cuti::logging_context_t& context) const
{
  ...
  if(endpoints.empty())
  {
    endpoints = x265_proto::default_endpoints(sockets_);
  }
...
void config_t::print_usage(std::ostream& os)
{
  ...
  os << "                                     (defaults:";
  {
    auto defaults = x265_proto::default_endpoints(sockets_);
